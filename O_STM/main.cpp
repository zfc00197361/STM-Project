...
void _warehouse_transfer_(std::shared_ptr<OSTM> _to_, std::shared_ptr<OSTM> _from_, TM& _tm, double _amount) {
    ...
    /*!
     * Clean up pointers after thread<br>
     */
    //tx->th_exit();
...
int main(void) {
     ...
     */
    //    std::cout << "[vector_number]" << std::endl;
    //    for (int i = 0; i < vector_number; ++i) {
    //        //_customer_vec[i]->toString();
    //        auto&& os = _customer_vec.at(i);
    //        os->toString();
    //    }
    //    std::cout << "[_warehouse_vec]" << std::endl;
    //    for(auto&& elem: _warehouse_vec){
    //        elem->toString(); // virtual dispatch
    //        
    //    }
    //_customer_vec[10]->toString();

    /*!
     * Clean up Transaction Manager from all main process associated pointers<br>
     * tm._TX_EXIT();
     */
    tm._TX_EXIT();
