...
The flow diagram \textit{Figure 2} introducing the dynamic relationship of the STM library. This relations interacting with the external application and the objects delegated to the STM library. In order to use the STM library the programmers must include it in the programmed module, to create an instance of a transaction, associated with the application by the Process ID to use the API interfaces. When the instances is created then the function calls are available to the library object. The key of the library usage is that the programmers can not instantiate more then one transaction object of the library per application, because the library will keep track of all process accessed the transaction manager by the process or process ID in the built in Data structure.
...
class Account : public Tobj{
	private:
			...
			Account(int amount, int accountID, string accountHolder){
				this->amount = amount;
