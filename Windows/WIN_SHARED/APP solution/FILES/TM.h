...
public:
     ...
     */
    static __declspec(dllexport)TM& Instance();
    /*!
     * \brief _get_tx std::shared_ptr<TX>, returning a shared pointer with the transaction
     */
	__declspec(dllexport) std::shared_ptr<TX>const _get_tx();
    /*!
     ...
     */
	__declspec(dllexport) void _TX_EXIT();
    /*!
     * ONLY FOR TESTING print_all void, prints all object in the txMap
     */
	__declspec(dllexport) void print_all();
