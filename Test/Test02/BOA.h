...
public:
     ...
     * Custom constructor
     */
    BOA(int accountNumber, double balance, std::string firstName, std::string lastName, std::string address) : BANK() {
     ...
     */
    BOA(std::shared_ptr<BANK> obj, int _version, int _unique_id) : BANK(_version, _unique_id) {
     ...
     */
    //virtual std::shared_ptr<BOA> _cast(std::shared_ptr<OSTM> _object);
