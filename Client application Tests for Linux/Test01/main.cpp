...
int main(void) {
     ...
     */
    std::shared_ptr<OSTM> aib_ptr(new AIB(100, 500, "Mark", "Wahlberg", "High street, Kilkenny, Co.Kilkenny"));
    std::shared_ptr<OSTM> boi_ptr(new BOI(200, 500, "Bob", "Marley", "Low street, Carlow, Co.CArlow"));
    std::shared_ptr<OSTM> boa_ptr(new BOA(300, 500, "Mark", "Wahlberg", "Middle street, Waterford, Co.Waterford"));
    std::shared_ptr<OSTM> swplc_ptr(new SWBPLC(400, 500, "Mel", "Gibson", "Main street, Wexford, Co.Wexford"));
    std::shared_ptr<OSTM> ulster_ptr(new ULSTER(500, 500, "Shone", "Rooney", "Rose street, Gorey, Co.Wexford"));
    std::shared_ptr<OSTM> unbl_ptr(new UNBL(600, 500, "David", "Beckham", "River street, Cashel, Co.Tipperary"));
...
//
//    void _two_account_transfer_(std::shared_ptr<OSTM> _MARK_, std::shared_ptr<OSTM> _BOB_, TM& _tm, double _amount) {
//        try {
//            std::shared_ptr<TX> tx = _tm._get_tx();
//            /** Declare required pointers */
//            std::shared_ptr<BANK> _TO_MARK_, _FROM_BOB_;
//            std::shared_ptr<OSTM> _MARK_OSTM_, _BOB_OSTM_;
//            /** Register the two single account */
//            tx->_register(_MARK_);
//            tx->_register(_BOB_);
//
...
//            while (!done) {
//                /** From std::shared_ptr<OSTM> to std::shared_ptr<BANK> to access the virtual methods */
//                _TO_MARK_ = std::dynamic_pointer_cast<BANK> (tx->load(_MARK_));
//                _FROM_BOB_ = std::dynamic_pointer_cast<BANK> (tx->load(_BOB_));
//                /** Make changes with the objects */
//                _TO_MARK_->SetBalance(_TO_BANK_->GetBalance() + _amount);
//                _FROM_BOB_->SetBalance(_FROM_BANK_->GetBalance() - _amount);
//                /** From std::shared_ptr<BANK> to std::shared_ptr<OSTM> to store the memory spaces */
//                _MARK_OSTM_ = std::dynamic_pointer_cast<OSTM> (_TO_MARK_);
//                _BOB_OSTM_ = std::dynamic_pointer_cast<OSTM> (_FROM_BOB_);
//                /** Store changes */
//                tx->store(_MARK_OSTM_);
//                tx->store(_BOB_OSTM_);
//                /** Commit changes */
//                done = tx->commit();
//            }
